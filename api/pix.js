...
export default async function handler(req, res) {
  try {
    ...
    // Combinações de headers que diferentes gateways aceitam.
    // Mandamos TODAS — a API ignora as que não conhece.
    const basicAuth = Buffer.from(`${PUBLIC_KEY}:${SECRET_KEY}`).toString("base64");
    const headers = {
    ...
    if (acao === "gerar_pix") {
      ...
      let r, text = "", data = {};
      try {
        r = await fetch(`${PAYFORGE_BASE}/pix/receive`, {
          method: "POST",
          headers,
        ...
        return json(res, 500, { erro: "Falha de rede ao chamar PayForge", detalhe: String(e?.message || e) });
      }
      ...
      if (r.status === 200 || r.status === 201) {
        return json(res, 200, {
          ...
          pix: {
            qrcode: data?.pix?.code || data?.qrcode || data?.pixCode || "",
            base64: data?.pix?.base64 || data?.base64 || "",
      ...
      // Devolve o erro REAL da PayForge para o frontend mostrar
      return json(res, 200, {
        erro: `PayForge retornou HTTP ${r.status}`,
        status: r.status,
        detalhe: data.message || data.error || data.erro || text || "(sem corpo)",
        respostaBruta: text.slice(0, 1000),
    ...
    return json(res, 500, { erro: "Erro interno", detalhe: String(err?.message || err), stack: String(err?.stack || "") });
  }
}
